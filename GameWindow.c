...
Game *New_Game() {
  ...
  game->title = "Final Project 112021116";
  game->display = NULL;
  game->game_init(game);
  return game;
}
...
void game_init(Game *self) {
  ...
  al_register_event_source(
      event_queue,
      al_get_display_event_source(self->display)); // register display event
  al_register_event_source(
      event_queue,
      al_get_keyboard_event_source()); // register keyboard event
  al_register_event_source(event_queue,
                           al_get_mouse_event_source()); // register mouse event
...
void game_destroy(Game *self) {
  ...
  char string_bfr[32];
  snprintf(string_bfr, sizeof(string_bfr), "%d", arr);
  al_set_config_value(cfg, "", "arr", string_bfr);
  snprintf(string_bfr, sizeof(string_bfr), "%d", das);
  al_set_config_value(cfg, "", "das", string_bfr);
  snprintf(string_bfr, sizeof(string_bfr), "%d", sdf);
  al_set_config_value(cfg, "", "sdf", string_bfr);
